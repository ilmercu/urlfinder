...
class TestValueMail(unittest.TestCase):
    def test_success_get_mail_with_protocol(self):
    ...
    def test_success_get_mail_without_protocol(self):
        main_url = 'abc@abc.com'
        url_parser = URLParser(main_url)
        mail = BaseElement(url_parser.get_parts())
        self.assertEqual(mail.get_value(), main_url)

    def test_success_get_mail_with_parameters(self):
        current_mail = 'abc@abc.com'
